...
public class VacancyControllerTest {
    ...
    @Test
    public void whenRequestVacanciesListPageThenGetPageWithVacancies() {
    ...
    @Test
    public void whenFoundVacancyByIdThenPageOne() {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
    ...
    @Test
    public void whenNotFoundVacancyThenGetErrorPage() {
        int invalidId = 1;
        when(vacancyService.findById(invalidId)).thenReturn(Optional.empty());

        var model = new ConcurrentModel();
        var view = vacancyController.getById(model, invalidId);
    ...
    @Test
    public void whenUpdatedVacancyThenTrueAndGetVacanciesPage() throws Exception {
        var updatedVacancy = new Vacancy(1, "test1", "new desc1", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
        when(vacancyService.update(
                vacancyArgumentCaptor.capture(), fileDtoArgumentCaptor.capture())).thenReturn(true);
